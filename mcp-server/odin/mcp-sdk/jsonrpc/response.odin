...
JSONRPC_Response :: struct {
  ...
  result:  Maybe(json.Value) `json:"result,omitempty"`,

  // if error: REQUIRED
  // if success: MUST NOT exist
  error:   Maybe(Response_Error) `json:"error,omitempty"`,
...
parse_response :: proc(
  ...
  if res.error != nil {
    ...
    if !ok {
      fmt.eprintfln("error struct is invalid: %+v", res.error)
      return {}, Response_Parse_Error.Error_Struct_Invalid
    }

    if !is_error_code_valid(error.code) {
      fmt.eprintfln("error code is not valid: %d", error.code)
